Public Class GUI_T_CommandManagement
  Public Shared TableName As String = "GUI_T_Command"
  ...
  Public Shared Function GetCommandDictionaryByReceiveSystem_WaitUUID(ByVal ReceiveSystem As enuSystemType, ByVal Wait_UUID As String) As Dictionary(Of String, clsFromGUICommand)
    ...
    Try
      ...
      If DBTool IsNot Nothing Then
        Dim strSQL As String = String.Empty
        Dim DatasetMessage As New DataSet
        strSQL = String.Format("Select * FROM {0} WHERE {1} IS NULL OR RESULT like '' AND {2}='{3}' AND {4}='{5}' ",
                                   TableName,
                                   IdxColumnName.RESULT.ToString,
  ...
  Private Shared Function SetInfoFromDB(ByRef Info As clsFromGUICommand, ByRef RowData As DataRow) As Boolean
    Try
      If RowData IsNot Nothing Then
        ...
        Info = New clsFromGUICommand(UUID, Send_System, Receive_System, Function_ID, SEQ, User_ID, Create_Time, Message, Result, Result_Message, Wait_UUID)
      End If
      Return True
    Catch ex As Exception
      SendMessageToLog(ex.ToString(), eCALogTool.ILogTool.enuTrcLevel.lvError)
