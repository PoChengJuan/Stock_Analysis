Public Class MCS_T_CommandManagement
  Public Shared TableName As String = "MCS_T_Command"
  ...
  Enum IdxColumnName As Integer
    ...
    WAIT_UUID
  End Enum
  Public Shared Function GetInsertSQL(ByRef Info As clsFromMCSCommand) As String
    Try
    ...
    End Try
  End Function
  Public Shared Function GetUpdateSQL(ByRef Info As clsFromMCSCommand) As String
    Try
      Dim strSQL As String = ""
    ...
    End Try
  End Function
  Public Shared Function GetDeleteSQL(ByRef Info As clsFromMCSCommand) As String
    Try
      Dim strSQL As String = ""
  ...
  Public Shared Function GetCommandDictionaryByReceiveSystem_ResultIsNULL_WaitUUIDIsNull(ByVal ReceiveSystem As enuSystemType) As Dictionary(Of String, clsFromMCSCommand)

    Try
      Dim ret_dic As New Dictionary(Of String, clsFromMCSCommand)
      If DBTool IsNot Nothing Then
        ...
        If DatasetMessage.Tables.Item(0).Rows.Count > 0 Then
          For RowIndex = 0 To DatasetMessage.Tables.Item(0).Rows.Count - 1
            Dim Info As clsFromMCSCommand = Nothing
            If SetInfoFromDB(Info, DatasetMessage.Tables.Item(0).Rows(RowIndex)) = True Then
              If Info IsNot Nothing Then
                ...
                End If
              Else
                SendMessageToLog("Get clsFromMCSCommand Info is nothing ", eCALogTool.ILogTool.enuTrcLevel.lvWARN)
              End If
            Else
              SendMessageToLog("Get clsFromMCSCommand Failed", eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            End If
          Next
        End If
      End If
  ...
  Public Shared Function GetCommandDictionaryByReceiveSystem_WaitUUID(ByVal ReceiveSystem As enuSystemType, ByVal Wait_UUID As String) As Dictionary(Of String, clsFromMCSCommand)
  ...
  Public Shared Function GetCommandDictionaryBySendSystem_ResultIsNotNULL(ByVal SendSystem As enuSystemType) As Dictionary(Of String, clsFromMCSCommand)
  ...
  End Function
  '-不要動
  Private Shared Function SetInfoFromDB(ByRef Info As clsFromMCSCommand, ByRef RowData As DataRow) As Boolean
    Try
      If RowData IsNot Nothing Then
        ...
        Info = New clsFromMCSCommand(UUID, Send_System, Receive_System, Function_ID, SEQ, User_ID, Create_Time, Message, Result, Result_Message, Wait_UUID)
      End If
      Return True
    Catch ex As Exception
      SendMessageToLog(ex.ToString(), eCALogTool.ILogTool.enuTrcLevel.lvError)
