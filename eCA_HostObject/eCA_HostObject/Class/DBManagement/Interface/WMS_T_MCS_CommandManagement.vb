Public Class WMS_T_MCS_CommandManagement
  Public Shared TableName As String = "WMS_T_MCS_Command"
  ...
  Enum IdxColumnName As Integer
    ...
    WAIT_UUID
  End Enum
  Public Shared Function GetInsertSQL(ByRef Info As clsToMCSCommand) As String
    Try
    ...
    End Try
  End Function
  Public Shared Function GetUpdateSQL(ByRef Info As clsToMCSCommand) As String
    Try
      Dim strSQL As String = ""
    ...
    End Try
  End Function
  Public Shared Function GetDeleteSQL(ByRef Info As clsToMCSCommand) As String
    Try
      Dim strSQL As String = ""
  ...
  Public Shared Function GetCommandDictionaryByReceiveSystem_ResultIsNULL_WaitUUIDIsNull(ByVal ReceiveSystem As enuSystemType) As Dictionary(Of String, clsToMCSCommand)

    Try
      Dim ret_dic As New Dictionary(Of String, clsToMCSCommand)
      If DBTool IsNot Nothing Then
        ...
        If DatasetMessage.Tables.Item(0).Rows.Count > 0 Then
          For RowIndex = 0 To DatasetMessage.Tables.Item(0).Rows.Count - 1
            Dim Info As clsToMCSCommand = Nothing
            If SetInfoFromDB(Info, DatasetMessage.Tables.Item(0).Rows(RowIndex)) = True Then
              If Info IsNot Nothing Then
                ...
                End If
              Else
                SendMessageToLog("Get clsToMCSCommand Info is nothing ", eCALogTool.ILogTool.enuTrcLevel.lvWARN)
              End If
            Else
              SendMessageToLog("Get clsToMCSCommand Failed", eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            End If
          Next
        End If
      End If
  ...
  Public Shared Function GetCommandDictionaryByReceiveSystem_WaitUUID(ByVal ReceiveSystem As enuSystemType, ByVal Wait_UUID As String) As Dictionary(Of String, clsToMCSCommand)
  ...
  Public Shared Function GetCommandDictionaryBySendSystem_ResultIsNotNULL(ByVal SendSystem As enuSystemType) As Dictionary(Of String, clsToMCSCommand)
  ...
  End Function
  '-不要動
  Private Shared Function SetInfoFromDB(ByRef Info As clsToMCSCommand, ByRef RowData As DataRow) As Boolean
    Try
      If RowData IsNot Nothing Then
        ...
        Info = New clsToMCSCommand(UUID, Send_System, Receive_System, Function_ID, SEQ, User_ID, Create_Time, Message, Result, Result_Message, Wait_UUID)
      End If
      Return True
    Catch ex As Exception
      SendMessageToLog(ex.ToString(), eCALogTool.ILogTool.enuTrcLevel.lvError)
