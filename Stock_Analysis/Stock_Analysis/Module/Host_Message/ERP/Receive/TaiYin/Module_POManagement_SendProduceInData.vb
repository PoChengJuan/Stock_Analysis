...
Module Module_POManagement_SendProduceInData
  Public Function O_POManagement_SendProduceInData(ByRef objPO As MSG_SendProduceInData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendProduceInData(enuRtnCode.Fail, PO_TYPE, PO_ID, "")
        Return False
      End If
      '取得SQL
      ...
      If Execute_DataUpdate(ret_strResultMsg, lstSql) = False Then
        ...
        Return False
      End If
      SendProduceInData(enuRtnCode.Sucess, PO_TYPE, PO_ID, "")
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendProduceInData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objProduceInDataInfo In objPO.ProduceInDataList.ProduceInDataInfo
        '檢查NoticeType是否為空
        If objProduceInDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查NoticeId是否為空
        If objProduceInDataInfo.POType = "" Then
          ret_strResultMsg = "POType is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objProduceInDetailDataInfo In objProduceInDataInfo.ProduceInDetailDataList.ProduceInDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objProduceInDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objProduceInDetailDataInfo.SKU = "" Then
            ret_strResultMsg = "SKU is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          'If objProduceInDetailDataInfo.Unit = "" Then
          '  ret_strResultMsg = "Unit is empty"
          '  SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          '  Return False
          'End If
          '檢查QTY是否為空
          If objProduceInDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "PickingQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objProduceInDetailDataInfo.CheckQty)
          If IsNumeric(objProduceInDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendProduceInData,
    ...
    Try
      ...
      For Each objProduceInDataInfo In objPO_Data.ProduceInDataList.ProduceInDataInfo
        Dim PO_ID As String = objProduceInDataInfo.POType & "_" & objProduceInDataInfo.POId
      ...
      For Each objProduceInDataInfo In objPO_Data.ProduceInDataList.ProduceInDataInfo
        ...
        Dim Warehouse = "" 'objProduceInDataInfo.Warehouse
        Dim PO_KEY1 = objProduceInDataInfo.POType
        Dim PO_KEY2 = objProduceInDataInfo.POId
        Dim PO_KEY3 = ""
        Dim PO_KEY4 = ""
        Dim PO_KEY5 = ""

        Dim PO_ID = PO_KEY1 & "_" & PO_KEY2 '領/退料單號
        ret_PO_ID = objProduceInDataInfo.POId
        Dim PO_TYPE1 = enuPOType_1.Combination_in
        Dim PO_TYPE2 = "" 'enuPOType_2.ProduceInData
        Dim PO_TYPE3 = ""
        Dim WO_TYPE = enuWOType.Receipt
        Dim H_PO_ORDER_TYPE = "" 'enuOrderType.ProduceInData
        Dim PRIORITY = 50
        'Dim CREATE_TIME = ""
        Dim START_TIME = ""
        Dim FINISH_TIME = ""
        ...
        Dim H_PO1 = objProduceInDataInfo.POType             '單別
        ret_PO_TYPE = H_PO1
        Dim H_PO2 = objProduceInDataInfo.ProduceInDateTime  '單據日期
        Dim H_PO3 = objProduceInDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = ""
        Dim H_PO7 = ""
        ...
        For Each objPO_DTL In objProduceInDataInfo.ProduceInDetailDataList.ProduceInDetailDataInfo
          '檢查料品主檔是否存在
          ...
          Dim TO_OWNER_ID = objProduceInDataInfo.FactoryId
          Dim TO_SUB_OWNER_ID = ""
          Dim FACTORY_ID = ""
          Dim DEST_AREA_ID = ""
...
#End Region
      ...
      'For Each objProduceInDataInfo In objPO_Data.ProduceInDataList.ProduceInDataInfo
      '  Dim PO_ID As String = objProduceInDataInfo.POId
      '  Dim PO_TYPE As String = objProduceInDataInfo.POType
      '  SendProduceInData(enuRtnCode.Sucess, PO_TYPE, PO_ID, ret_strResultMsg)
      'Next
