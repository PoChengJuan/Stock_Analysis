...
Module Module_POManagement_SendPickUpData
  Public Function O_POManagement_SendPickUpData(ByRef objPO As MSG_SendPickUpData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendPickUpData(enuRtnCode.Fail, PO_TYPE, PO_ID, ret_strResultMsg)
        Return False
      End If
      '取得SQL
      ...
      If Execute_DataUpdate(ret_strResultMsg, lstSql) = False Then
        ...
        Return False
      End If
      SendPickUpData(enuRtnCode.Sucess, PO_TYPE, PO_ID, ret_strResultMsg)
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendPickUpData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objPickUpDataInfo In objPO.PickUpDataList.PickUpDataInfo
        '檢查NoticeType是否為空
        If objPickUpDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查NoticeId是否為空
        If objPickUpDataInfo.POType = "" Then
          ret_strResultMsg = "POType is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objPickupDetailDataInfo In objPickUpDataInfo.PickUpDetailDataList.PickUpDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objPickupDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objPickupDetailDataInfo.SKU = "" Then
            ret_strResultMsg = "SKU is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          'If objPickupDetailDataInfo.Unit = "" Then
          '  ret_strResultMsg = "Unit is empty"
          '  SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          '  Return False
          'End If
          '檢查QTY是否為空
          If objPickupDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "CheckQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objPickupDetailDataInfo.CheckQty)
          If IsNumeric(objPickupDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendPickUpData,
    ...
    Try
      ...
      For Each objPickUpDataInfo In objPO_Data.PickUpDataList.PickUpDataInfo
        Dim PO_ID As String = objPickUpDataInfo.POType & "_" & objPickUpDataInfo.POId
      ...
      For Each objPickUpDataInfo In objPO_Data.PickUpDataList.PickUpDataInfo
        ...
        Dim PO_ID = objPickUpDataInfo.POType & "_" & objPickUpDataInfo.POId  '領/退料單號
        ret_PO_ID = objPickUpDataInfo.POId
        Dim PO_TYPE1 = enuPOType_1.Picking_out
        Dim PO_TYPE2 = "301"
        Dim PO_TYPE3 = ""
        Dim WO_TYPE = enuWOType.Discharge
        Dim H_PO_ORDER_TYPE = "301"
        PickingData_TYPE = ""
        ...
        Dim H_PO1 = objPickUpDataInfo.POType             '單別
        ret_PO_TYPE = objPickUpDataInfo.POType
        Dim H_PO2 = objPickUpDataInfo.PickUpTime    '單據日期
        Dim H_PO3 = objPickUpDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = ""
        Dim H_PO7 = ""
        ...
        For Each objPO_DTL In objPickUpDataInfo.PickUpDetailDataList.PickUpDetailDataInfo
          '檢查料品主檔是否存在
          ...
          Dim ITEM_COMMON4 = objPickUpDataInfo.FactoryId
          Dim ITEM_COMMON5 = ""
          Dim ITEM_COMMON6 = ""
          Dim ITEM_COMMON7 = ""
          Dim ITEM_COMMON8 = ""
