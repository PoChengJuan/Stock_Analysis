...
Module Module_POManagement_SendInboundData
  Public Function O_POManagement_SendInboundData(ByRef objPO As MSG_SendInboundData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendInboundData(enuRtnCode.Fail, PO_TYPE, PO_ID, "")
        Return False
      End If
      '取得SQL
      ...
      If Execute_DataUpdate(ret_strResultMsg, lstSql) = False Then
        ...
        Return False
      End If
      SendInboundData(enuRtnCode.Sucess, PO_TYPE, PO_ID, "")
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendInboundData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objInboundDataInfo In objPO.InboundDataList.InboundDataInfo
        '檢查NoticeType是否為空
        If objInboundDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查NoticeId是否為空
        If objInboundDataInfo.POType = "" Then
          ret_strResultMsg = "POType is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objInboundDetailDataInfo In objInboundDataInfo.InboundDetailDataList.InboundDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objInboundDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objInboundDetailDataInfo.SKU = "" Then
          ...
          'End If
          '檢查QTY是否為空
          If objInboundDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "PickingQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objInboundDetailDataInfo.CheckQty)
          If IsNumeric(objInboundDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendInboundData,
    ...
    Try
      ...
      For Each objInboundDataInfo In objPO_Data.InboundDataList.InboundDataInfo
        Dim PO_ID As String = objInboundDataInfo.POType & "_" & objInboundDataInfo.POId
      ...
      For Each objInboundDataInfo In objPO_Data.InboundDataList.InboundDataInfo
        ...
        Dim PO_KEY1 = objInboundDataInfo.POType
        Dim PO_KEY2 = objInboundDataInfo.POId
        Dim PO_KEY3 = ""
        Dim PO_KEY4 = ""
        Dim PO_KEY5 = ""

        Dim PO_ID = PO_KEY1 & "_" & PO_KEY2  '領/退料單號
        ret_PO_ID = objInboundDataInfo.POId
        ...
        Dim H_PO1 = objInboundDataInfo.POType             '單別
        ret_PO_TYPE = objInboundDataInfo.POType
        Dim H_PO2 = objInboundDataInfo.InboundDateTime  '單據日期
        Dim H_PO3 = objInboundDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = ""
        Dim H_PO7 = ""
        ...
        For Each objPO_DTL In objInboundDataInfo.InboundDetailDataList.InboundDetailDataInfo
          '檢查料品主檔是否存在
          ...
          Dim TO_OWNER_ID = objInboundDataInfo.FactoryId
          Dim TO_SUB_OWNER_ID = ""
          Dim FACTORY_ID = ""
          Dim DEST_AREA_ID = ""
...
#Region "一般單據的PO_DTL"
          If tmp_dicPO_DTL.ContainsKey(PO_DTL_Key) = True Then  '單據存在
            ...
            With objNewPO_DTL
              ...
              .LOT_NO = LOT_NO
              .QTY = QTY
              .ITEM_COMMON1 = ITEM_COMMON1
              .ITEM_COMMON2 = ITEM_COMMON2
              .H_POD1 = H_POD1
              .H_POD2 = H_POD2
              .H_POD3 = H_POD3
              .H_POD4 = H_POD4
