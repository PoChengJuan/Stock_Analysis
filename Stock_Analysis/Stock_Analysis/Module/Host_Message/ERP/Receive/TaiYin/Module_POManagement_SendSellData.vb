...
Module Module_POManagement_SendSellData
  Public Function O_POManagement_SendOutData(ByRef objPO As MSG_SendOutData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendSellData(enuRtnCode.Fail, PO_TYPE, PO_ID, "")
        Return False
      End If
      '取得SQL
      ...
      If Execute_DataUpdate(ret_strResultMsg, lstSql) = False Then
        ...
        Return False
      End If
      SendSellData(enuRtnCode.Sucess, PO_TYPE, PO_ID, "")
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendOutData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objOutDataInfo In objPO.OutDataList.OutDataInfo
        '檢查NoticeType是否為空
        If objOutDataInfo.PoID = "" Then
          ret_strResultMsg = "FromID is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查NoticeId是否為空
        If objOutDataInfo.POType = "" Then
          ret_strResultMsg = "FromHead is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objOutDetailDataInfo In objOutDataInfo.OutDetailDataList.OutDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objOutDetailDataInfo.SerialID = "" Then
            ret_strResultMsg = "SerialID is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objOutDetailDataInfo.SKU = "" Then
          ...
          'End If
          '檢查QTY是否為空
          If objOutDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "CheckQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '用QTY檢查的話非數字被CINT就會盡EXCEPTIOIN，所以改成直接丟去判斷就好
          'Dim QTY = CInt(objOutDetailDataInfo.CheckQty)
          If IsNumeric(objOutDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendOutData,
    ...
    Try
      ...
      For Each objoutDataInfo In objPO_Data.OutDataList.OutDataInfo
        Dim PO_ID As String = objoutDataInfo.POType & "_" & objoutDataInfo.PoID
      ...
      End If
      For Each objOutDataInfo In objPO_Data.OutDataList.OutDataInfo
        'Dim Warehouse = objOutDataInfo.Warehouse
...
#Region "EventID變動區"
        ...
        Select Case objPO_Data.EventID
          Case "SendSellData"
            PO_TYPE2 = enuPOType_2.Sell_Out
            H_PO_ORDER_TYPE = enuOrderType.Sell_Out
          Case "SendPickUpData"
            PO_TYPE2 = enuPOType_2.Picking_Material_Out
            H_PO_ORDER_TYPE = enuOrderType.Picking_Material_Out
...
#End Region
        ...
        Dim PO_KEY1 = objOutDataInfo.POType
        Dim PO_KEY2 = objOutDataInfo.PoID
        Dim PO_KEY3 = ""
        Dim PO_KEY4 = ""
        Dim PO_KEY5 = ""

        Dim PO_ID = PO_KEY1 & "_" & PO_KEY2 '
        ret_PO_ID = objOutDataInfo.PoID
        Dim PO_TYPE1 = enuPOType_1.Picking_out
        'PO_TYPE2是變動欄位
        Dim PO_TYPE3 = ""
        Dim WO_TYPE = enuWOType.Discharge
        'H_PO_ORDER_TYPE是變動欄位
        PickingData_TYPE = ""
        ...
        Dim H_PO1 = objOutDataInfo.POType             '單別
        ret_PO_TYPE = objOutDataInfo.POType
        Dim H_PO2 = objOutDataInfo.OutDateTime    '單據日期
        Dim H_PO3 = objOutDataInfo.FactoryID          '廠別代號
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = ""
        Dim H_PO7 = ""
        ...
        For Each objPO_DTL In objOutDataInfo.OutDetailDataList.OutDetailDataInfo
          '檢查料品主檔是否存在
          ...
          If gMain.objHandling.O_GetDB_dicSKUBySKUNo(SKU_NO, dicSKU) = True Then
            ...
            Else
              ret_strResultMsg = "品號不存在無法取得庫別資訊, PO_ID=" & PO_ID & ", SKU_NO=" & SKU_NO & " 請先建立品號資料"
              SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
              Return False
            End If
          End If
...
#Region "EventID變動區"
          ...
          Select Case objPO_Data.EventID
            ...
            Case "SendPickUpData"
              ITEM_COMMON3 = objPO_DTL.Item_Common3
            Case Else
