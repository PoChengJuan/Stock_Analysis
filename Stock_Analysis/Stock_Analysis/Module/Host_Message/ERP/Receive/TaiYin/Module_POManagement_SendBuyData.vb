...
Module Module_POManagement_SendBuyData
  Public Function O_POManagement_SendInData(ByRef objPO As MSG_SendInData,
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendInData, ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objInDataInfo In objPO.InDataList.InDataInfo
        ''檢查NoticeType是否為空
        'If objInDataInfo.FromID = "" Then
        '  ret_strResultMsg = "FromID is empty"
        '  SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
        '  Return False
        'End If
        ''檢查NoticeId是否為空
        'If objInDataInfo.FromHead = "" Then
        '  ret_strResultMsg = "FromHead is empty"
        '  SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
        '  Return False
        'End If
        For Each objInDetailDataInfo In objInDataInfo.InDetailDataList.InDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objInDetailDataInfo.SerialID = "" Then
          ...
          '檢查SKU是否為空
          If objInDetailDataInfo.SKU = "" Then
            ret_strResultMsg = "SKU is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          'If objBuyDetailDataInfo.Unit = "" Then
          '  ret_strResultMsg = "Unit is empty"
          '  SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          '  Return False
          'End If
          '檢查QTY是否為空
          If objInDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "CheckQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If

          '這邊的寫法有問題，非數字進來CINT 應該會EXCEPTION，應該直接用IsNumeric(objInDetailDataInfo.CheckQty)
          'Dim QTY = CInt(objInDetailDataInfo.CheckQty)
          If IsNumeric(objInDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendInData,
    ...
    Try
      ...
      For Each objBuyDataInfo In objPO_Data.InDataList.InDataInfo
        Dim PO_ID As String = objBuyDataInfo.POType & "_" & objBuyDataInfo.POID
      ...
      If gMain.objHandling.O_GetDB_dicPODTLTRANSACTIONBydicPO_ID(tmp_dicPOID, tmp_dicPO_DTL_TRANSACTION) = False Then
        ...
        Return False
      End If
      For Each objInDataInfo In objPO_Data.InDataList.InDataInfo
        'Dim obj = objNoticeDataInfo.NoticeDetailDataList.NoticeDetailDataInfo.First

#Region "隨EventID變動區"
        '隨EventID變動區
        '以下會隨不同EventID而有變動的欄位
        Dim PO_TYPE2 = Nothing
        Dim H_PO_ORDER_TYPE = Nothing

        Select Case objPO_Data.EventID
          Case "SendInboundData"
            PO_TYPE2 = enuPOType_2.Inbound_Data
            H_PO_ORDER_TYPE = enuOrderType.Inbound_Data
          Case "SendProduceInData"
            PO_TYPE2 = enuPOType_2.Product_In
            H_PO_ORDER_TYPE = enuOrderType.Product_In
          Case "SendReturnData"
            'PO_TYPE2 = enuPOType_2.Pick_Material_back
            'H_PO_ORDER_TYPE = enuOrderType.Pick_Material_back
          Case "SendSellReturnData"
            PO_TYPE2 = enuPOType_2.Sell_Back
            H_PO_ORDER_TYPE = enuOrderType.Sell_Back
          Case "SendOtherInData"
            PO_TYPE2 = enuPOType_2.Other_In
            H_PO_ORDER_TYPE = enuOrderType.Other_In
          Case Else
            PO_TYPE2 = enuPOType_2.Unknow
            H_PO_ORDER_TYPE = enuOrderType.Unknow
        End Select
#End Region

        '以下是填固定對應欄位
        Dim PO_ID = objInDataInfo.POType & "_" & objInDataInfo.POID  '領/退料單號
        ret_PO_ID = objInDataInfo.POID
        ret_PO_TYPE = objInDataInfo.POType
        ...
        Dim H_PO1 = objInDataInfo.InDateTime.Trim()
        Dim H_PO2 = objInDataInfo.FactoryID.Trim()
        Dim H_PO3 = ""
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = ""
        ...
        For Each objPO_DTL In objInDataInfo.InDetailDataList.InDetailDataInfo
          Dim ASRSPart = "N"
          '檢查料品主檔是否存在
