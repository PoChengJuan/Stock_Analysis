...
Module Module_POManagement_SendInboundReturnData
  Public Function O_POManagement_SendInboundReturnData(ByRef objPO As MSG_SendInboundReturnData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendInbound_ReturnData(enuRtnCode.Fail, PO_TYPE, PO_ID, "")
        Return False
      End If
      '取得SQL
      ...
      If Execute_DataUpdate(ret_strResultMsg, lstSql) = False Then
        ...
        Return False
      End If
      SendInbound_ReturnData(enuRtnCode.Sucess, PO_TYPE, PO_ID, "")
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendInboundReturnData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objInboundReturnDataInfo In objPO.InboundReturnDataList.InboundReturnDataInfo
        '檢查NoticeType是否為空
        If objInboundReturnDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查NoticeId是否為空
        If objInboundReturnDataInfo.POType = "" Then
          ret_strResultMsg = "POType is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objInboundReturnDetailDataInfo In objInboundReturnDataInfo.InboundReturnDetailDataList.InboundReturnDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objInboundReturnDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objInboundReturnDetailDataInfo.SKU = "" Then
            ret_strResultMsg = "SKU is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          'If objInboundDetailDataInfo.Unit = "" Then
          '  ret_strResultMsg = "Unit is empty"
          '  SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          '  Return False
          'End If
          '檢查QTY是否為空
          If objInboundReturnDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "PickingQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objInboundReturnDetailDataInfo.CheckQty)
          If IsNumeric(objInboundReturnDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendInboundReturnData,
    ...
    Try
      ...
      For Each objInboundReturnDataInfo In objPO_Data.InboundReturnDataList.InboundReturnDataInfo
        Dim PO_ID As String = objInboundReturnDataInfo.POType & "_" & objInboundReturnDataInfo.POId
      ...
      For Each objInboundReturnDataInfo In objPO_Data.InboundReturnDataList.InboundReturnDataInfo
        ...
        Dim Warehouse = objInboundReturnDataInfo.Warehouse
        Dim PO_KEY1 = objInboundReturnDataInfo.POType
        Dim PO_KEY2 = objInboundReturnDataInfo.POId
        Dim PO_KEY3 = ""
        Dim PO_KEY4 = ""
        Dim PO_KEY5 = ""

        Dim PO_ID = PO_KEY1 & "_" & PO_KEY2  '領/退料單號
        ret_PO_ID = objInboundReturnDataInfo.POId
        Dim PO_TYPE1 = enuPOType_1.Picking_out
        Dim PO_TYPE2 = "" 'enuPOType_2.InboundReturn_Data
        Dim PO_TYPE3 = ""
        Dim WO_TYPE = enuWOType.Discharge
        Dim H_PO_ORDER_TYPE = "" 'enuOrderType.InboundReturn_Data
        Dim PRIORITY = "50"
        'Dim CREATE_TIME = ""
        Dim START_TIME = ""
        Dim FINISH_TIME = ""
        ...
        Dim H_PO1 = objInboundReturnDataInfo.POType             '單別
        ret_PO_TYPE = objInboundReturnDataInfo.POType
        Dim H_PO2 = objInboundReturnDataInfo.InboundReturnDateTime  '單據日期
        Dim H_PO3 = objInboundReturnDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = ""
        Dim H_PO7 = ""
        ...
        For Each objPO_DTL In objInboundReturnDataInfo.InboundReturnDetailDataList.InboundReturnDetailDataInfo
          '檢查料品主檔是否存在
          ...
          Dim FROM_OWNER_ID = objInboundReturnDataInfo.FactoryId
...
#End Region
      ...
      'For Each objInboundDataInfo In objPO_Data.InboundDataList.InboundDataInfo
      '  Dim PO_ID As String = objInboundDataInfo.POId
      '  Dim PO_TYPE As String = objInboundDataInfo.POType
