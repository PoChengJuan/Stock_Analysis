...
Module Module_POManagement_SendTransferData
  Public Function O_POManagement_SendTransferData(ByRef objPO As MSG_SendTransferData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, DocTypeCode, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION, dicGluePO_DTL) = False Then
        ...
        'End If
        SendTransactionData(enuRtnCode.Fail, PO_TYPE, PO_ID, "")
        Return False
      End If
      '取得SQL
...
#Region "自己將PO執行"
      ...
      'If Get_PO_TO_WO(dicAdd_PO, ret_strResultMsg, dicAdd_PO_DTL, Host_Command, "") = False Then
      '  'SendTransactionData(enuRtnCode.Fail, PO_TYPE, PO_ID, ret_strResultMsg)
      '  Return False
      'End If
      ''取得SQL
...
#End Region
      ...
      'End If
      SendTransactionData(enuRtnCode.Sucess, PO_TYPE, PO_ID, "")
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendTransferData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objTransferDataInfo In objPO.TransferDataList.TransferDataInfo
        '檢查POId是否為空
        If objTransferDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查POType是否為空
        If objTransferDataInfo.POType = "" Then
        ...
        '  Return False
        'End If
        For Each objTransactionDetailDataInfo In objTransferDataInfo.TransferDetailDataList.TransferDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objTransactionDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objTransactionDetailDataInfo.SKU = "" Then
          ...
          'End If
          '檢查QTY是否為空
          If objTransactionDetailDataInfo.CheckQty = "" Then
            ret_strResultMsg = "CheckQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objTransactionDetailDataInfo.CheckQty)
          If IsNumeric(objTransactionDetailDataInfo.CheckQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByRef DocTypeCode As String, ByVal objPO_Data As MSG_SendTransferData,
    ...
    Try
      ...
      For Each objTransactionDataInfo In objPO_Data.TransferDataList.TransferDataInfo
        Dim PO_ID As String = objTransactionDataInfo.POType & "_" & objTransactionDataInfo.POId
      ...
      For Each objTransactionDataInfo In objPO_Data.TransferDataList.TransferDataInfo
        ...
        Dim PO_KEY1 = objTransactionDataInfo.POType
        Dim PO_KEY2 = objTransactionDataInfo.POId
        Dim PO_KEY3 = ""
        Dim PO_KEY4 = ""
        Dim PO_KEY5 = ""

        Dim PO_ID = PO_KEY1 & "_" & PO_KEY2  '領/退料單號
        ret_PO_ID = objTransactionDataInfo.POId
        Dim PO_TYPE1 = ""
        Dim PO_TYPE2 = ""
        Dim PO_TYPE3 = ""
        Dim WO_TYPE = ""
        ...
        Dim H_PO1 = objTransactionDataInfo.POType             '單別
        ret_PO_TYPE = H_PO1
        Dim H_PO2 = objTransactionDataInfo.TransferDateTime    '單據日期
        Dim H_PO3 = objTransactionDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""    '生產線別
        Dim H_PO5 = ""              '單據性質
        Dim H_PO6 = ""       '確認碼
        Dim H_PO7 = objTransactionDataInfo.TransferOutWarehouse '轉出庫
        Dim H_PO8 = objTransactionDataInfo.TransferInWarehouse  '轉入庫
        Dim H_PO9 = ""
        Dim H_PO10 = ""
        Dim H_PO11 = ""
        Dim H_PO12 = ""
        ...
        If gMain.objHandling.O_GetDB_dicSLByAll(tmp_dicSL) = False Then
          ret_strResultMsg = "Get SL data From DB Failed"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        Dim Flg_In = False
        ...
        'TODO IN_OUT的判斷要再CHECK一下
        If objTransactionDataInfo.TransferInWarehouse = ASRSDevice Then
          Flg_Out = True
        End If
        If objTransactionDataInfo.TransferOutWarehouse = ASRSDevice Then
          Flg_In = True
        End If

        '只能IN / OUT 其中一邊是TRUE
        If Flg_In = False And Flg_Out = False Then
          ret_strResultMsg = "TransferWarehouse All Not AS/RS"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        ElseIf Flg_In = True And Flg_Out = True Then
          ret_strResultMsg = "TransferWarehouse All AS/RS"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        ElseIf Flg_In = True Then
          '轉撥入
          PO_TYPE1 = enuPOType_1.Combination_in
          PO_TYPE2 = enuPOType_2.Transaction_in
          WO_TYPE = enuWOType.Receipt
          H_PO_ORDER_TYPE = enuOrderType.Transaction_in
        ElseIf Flg_Out = True Then
          '轉撥出
          PO_TYPE1 = enuPOType_1.Picking_out
          PO_TYPE2 = enuPOType_2.transaction_out
          WO_TYPE = enuWOType.Discharge
          H_PO_ORDER_TYPE = enuOrderType.Transaction_out
        End If
        'If objTransactionDataInfo.TransferOutWarehouse = "01" Then
        '  '轉撥出
        '  PO_TYPE1 = enuPOType_1.Picking_out
        '  PO_TYPE2 = enuPOType_2.transaction_out
        '  WO_TYPE = enuWOType.Discharge
        '  H_PO_ORDER_TYPE = enuOrderType.transaction_out
        'ElseIf objTransactionDataInfo.TransferInWarehouse = "01" Then
        '  '轉撥入
        '  PO_TYPE1 = enuPOType_1.Combination_in
        '  PO_TYPE2 = enuPOType_2.transaction_in
        '  WO_TYPE = enuWOType.Receipt
        ...
        For Each objPO_DTL In objTransactionDataInfo.TransferDetailDataList.TransferDetailDataInfo
          '檢查料品主檔是否存在
          ...
          Dim SKU_NO = objPO_DTL.SKU & "_" & "TYM" & "_" & "ERP"
          ...
          Dim PACKAGE_ID = "" 'objPO_DTL.SN
          Dim ITEM_COMMON1 = objPO_DTL.Item_Common1
          Dim ITEM_COMMON2 = objPO_DTL.Item_Common2
          ...
          If PO_TYPE1 = enuPOType_1.Combination_in Then
            ITEM_COMMON10 = H_PO8
          ElseIf PO_TYPE1 = enuPOType_1.Picking_out Then
            ITEM_COMMON10 = H_PO7
          End If
          ...
          Dim SORT_ITEM_COMMON5 = objPO_DTL.Item_Common3
          Dim COMMENTS = ""
          Dim EXPIRED_DATE = ""
          ...
          Dim TO_SUB_OWNER_ID = ""
          If PO_TYPE1 = enuPOType_1.Combination_in Then
            TO_OWNER_ID = objTransactionDataInfo.Owner
          Else
            FROM_OWNER_ID = objTransactionDataInfo.Owner
          End If
          Dim FACTORY_ID = ""
          Dim DEST_AREA_ID = ""
