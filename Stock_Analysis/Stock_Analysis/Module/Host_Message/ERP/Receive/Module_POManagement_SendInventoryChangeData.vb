...
Module Module_POManagement_SendInventoryChangeData
  Public Function O_POManagement_SendInventoryChangeData(ByRef objPO As MSG_SendInventoryChangeData,
    ...
    Try
      ...
      End If
      '進行資料調整
      If Get_UpdateData(PO_ID, PO_TYPE, DocTypeCode, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendInventoryChangeData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objTransactionDataInfo In objPO.TransactionDataList.TransactionDataInfo
        '檢查POId是否為空
        If objTransactionDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查POType是否為空
        If objTransactionDataInfo.POType = "" Then
        ...
        If objTransactionDataInfo.DocTypeCode = "" Then
          ...
          Return False
        End If
        For Each objTransactionDetailDataInfo In objTransactionDataInfo.TransactionDetailDataList.TransactionDetailDataInfo
          ...
          End If
          '檢查QTY是否為空
          If objTransactionDetailDataInfo.Qty = "" Then
            ret_strResultMsg = "PickingQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objTransactionDetailDataInfo.Qty)
          If IsNumeric(objTransactionDetailDataInfo.Qty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByRef DocTypeCode As String, ByVal objPO_Data As MSG_SendInventoryChangeData,
    ...
    Try
      ...
      For Each objTransactionDataInfo In objPO_Data.TransactionDataList.TransactionDataInfo
        ...
        Dim PO_ID = objTransactionDataInfo.POType & "_" & objTransactionDataInfo.POId  '領/退料單號
        ret_PO_ID = objTransactionDataInfo.POId
        Dim PO_TYPE1 = ""
        Dim PO_TYPE2 = ""
        Dim PO_TYPE3 = objTransactionDataInfo.DocTypeCode
        DocTypeCode = PO_TYPE3
        Dim WO_TYPE = ""
        Dim H_PO_ORDER_TYPE = ""
        ' PickingData_TYPE = objobjTransactionDataInfo.Kind
        Select Case PO_TYPE3
          Case "11"    '庫內領料
            SendMessageToLog("庫存異動", eCALogTool.ILogTool.enuTrcLevel.lvTRACE)
            bln_DocTypeisNormal = True
            'Return True
          Case "12"     '託外領料
            bln_DocTypeisNormal = False
          Case Else
        ...
        ret_PO_TYPE = H_PO1
        Dim H_PO2 = objTransactionDataInfo.TransactionDateTime    '單據日期
        Dim H_PO3 = objTransactionDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""    '生產線別
        Dim H_PO5 = ""              '單據性質
        Dim H_PO6 = objTransactionDataInfo.ConfirmCode        '確認碼
        Dim H_PO7 = objTransactionDataInfo.DocTypeCode        '單據性質
        Dim H_PO8 = ""
        Dim H_PO9 = ""
        Dim H_PO10 = ""
        Dim H_PO11 = ""
        ...
        For Each objPO_DTL In objTransactionDataInfo.TransactionDetailDataList.TransactionDetailDataInfo
          '檢查料品主檔是否存在
          ...
          Dim QTY As Integer = CInt(objPO_DTL.Qty)
          QTY = Abs(QTY)  '取絕對值
          ...
          If bln_DocTypeisNormal = True Then
            '一般單據
            If IntegerCheckPositive(objPO_DTL.Qty) = True Then
              '正數 入庫
              PO_TYPE1 = enuPOType_1.Combination_in
          ...
          Else
            '轉撥單
            If objPO_DTL.TransferOutWarehouse = "A100" Then
              '轉撥出
              PO_TYPE1 = enuPOType_1.Picking_out
              PO_TYPE2 = "" 'enuPOType_2.transaction_out
              WO_TYPE = enuWOType.Discharge
              H_PO_ORDER_TYPE = "" 'enuOrderType.transaction_out
            ElseIf objPO_DTL.TransferInWarehouse = "A100" Then
              '轉撥入
              PO_TYPE1 = enuPOType_1.Combination_in
          ...
          Dim H_POD6 = ""
          Dim H_POD7 = ""
          Dim H_POD8 = objPO_DTL.TransferOutWarehouse '轉出庫
          Dim H_POD9 = objPO_DTL.TransferInWarehouse '轉入庫
          Dim H_POD10 = objPO_DTL.TransferOutShelfId  '轉出儲位
          Dim H_POD11 = objPO_DTL.TransferInShelfId '轉入儲位
          Dim H_POD12 = ""
          Dim H_POD13 = ""
          Dim H_POD14 = ""
          Dim H_POD15 = ""
