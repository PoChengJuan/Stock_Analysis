...
Module Module_POManagement_SendTempInOutReturnData
  Public Function O_POManagement_SendTempInOutReturnData(ByRef objPO As MSG_SendTempInOutrReturnData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendTempInOutData(enuRtnCode.Fail, PO_TYPE, PO_ID, ret_strResultMsg)
        Return False
      End If
      '取得SQL
      ...
      If Execute_DataUpdate(ret_strResultMsg, lstSql) = False Then
        ...
        Return False
      End If
      SendTempInOutData(enuRtnCode.Sucess, PO_TYPE, PO_ID, ret_strResultMsg)
      Return True
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendTempInOutrReturnData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objTempInOutDataInfo In objPO.TempInOutDataList.TempInOutDataInfo
        '檢查POId是否為空
        If objTempInOutDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查POType是否為空
        If objTempInOutDataInfo.POType = "" Then
          ret_strResultMsg = "POType is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查DocTypeCode是否為空
        If objTempInOutDataInfo.ConfirmCode = "" Then
          ret_strResultMsg = "ConfirmCode is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objTempInOutDetailDataInfo In objTempInOutDataInfo.TempInOutDetailDataList.TempInOutDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objTempInOutDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objTempInOutDetailDataInfo.SKU = "" Then
            ret_strResultMsg = "SKU is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objTempInOutDetailDataInfo.Unit = "" Then
            ret_strResultMsg = "Unit is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查QTY是否為空
          If objTempInOutDetailDataInfo.Qty = "" Then
            ret_strResultMsg = "PickingQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objTempInOutDetailDataInfo.Qty)
          If IsNumeric(objTempInOutDetailDataInfo.Qty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendTempInOutrReturnData,
    ...
    Try
      ...
      For Each objTempInOutDataInfo In objPO_Data.TempInOutDataList.TempInOutDataInfo
        Dim PO_ID As String = objTempInOutDataInfo.POType & "_" & objTempInOutDataInfo.POId
      ...
      For Each objTempInOutDataInfo In objPO_Data.TempInOutDataList.TempInOutDataInfo
        ...
        Dim PO_ID = objTempInOutDataInfo.POType & "_" & objTempInOutDataInfo.POId  '領/退料單號
        ret_PO_ID = objTempInOutDataInfo.POId
        Dim PO_TYPE1 = ""
        Dim PO_TYPE2 = ""
        Dim PO_TYPE3 = ""
        Dim WO_TYPE = ""
        ...
        Dim H_PO1 = objTempInOutDataInfo.POType             '單別
        ret_PO_TYPE = H_PO1
        Dim H_PO2 = objTempInOutDataInfo.TempInOutDateTime    '單據日期
        Dim H_PO3 = objTempInOutDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = ""
        Dim H_PO5 = ""
        Dim H_PO6 = objTempInOutDataInfo.ConfirmCode        '確認碼
        Dim H_PO7 = ""
        Dim H_PO8 = ""
        Dim H_PO9 = ""
        Dim H_PO10 = ""
        ...
        For Each objPO_DTL In objTempInOutDataInfo.TempInOutDetailDataList.TempInOutDetailDataInfo
          '檢查料品主檔是否存在
          ...
          If SetQTYByPackeUnit(objPO_DTL.SKU, objPO_DTL.Qty, objPO_DTL.Unit, QTY, ret_strResultMsg) = False Then
            Return False
          End If
          If objPO_DTL.TransferInWarehouse = "110" Then
            '入庫
            PO_TYPE1 = enuPOType_1.Combination_in
            PO_TYPE2 = "" 'enuPOType_2.temp_out_return
            WO_TYPE = enuWOType.Receipt
            H_PO_ORDER_TYPE = "" 'enuOrderType.temp_out_return
          Else
            '負數 出庫
            PO_TYPE1 = enuPOType_1.Picking_out
            PO_TYPE2 = "" 'enuPOType_2.temp_in_return
            WO_TYPE = enuWOType.Discharge
            H_PO_ORDER_TYPE = "" 'enuOrderType.temp_in_return
          End If
          Dim QTY_FINISH = 0
          Dim H_QTY_PROCESS = 0
          Dim H_POL1 = ""
...
#End Region
      ...
      'For Each objTempInOutDataInfo In objPO_Data.TempInOutDataList.TempInOutDataInfo
      '  Dim PO_ID As String = objTempInOutDataInfo.POId
      '  Dim PO_TYPE As String = objTempInOutDataInfo.POType
      '  SendTempInOutData(enuRtnCode.Sucess, PO_TYPE, PO_ID, ret_strResultMsg)
      'Next
