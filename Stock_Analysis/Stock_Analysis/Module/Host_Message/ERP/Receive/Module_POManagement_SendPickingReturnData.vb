...
Module Module_POManagement_SendPickingReturnData
  Public Function O_POManagement_SendPickingReturnData(ByRef objPO As MSG_SendPickingReturnData,
    ...
    Try
      ...
      If Get_UpdateData(PO_ID, PO_TYPE, objPO, ret_strResultMsg, Host_Command, dicAdd_PO, dicUpdate_PO, dicAdd_PO_Line, dicDelete_PO_Line, dicUpdate_PO_Line, dicAdd_PO_DTL, dicDelete_PO_DTL, dicUpdate_PO_DTL, dicAdd_PO_DTL_TRANSACTION, dicDelete_PO_DTL_TRANSACTION, dicUpdate_PO_DTL_TRANSACTION) = False Then
        SendPickingData(enuRtnCode.Fail, PO_TYPE, PO_ID, ret_strResultMsg)
        Return False
      End If
      '取得SQL
  ...
  Private Function Check_Data(ByVal objPO As MSG_SendPickingReturnData,
                                                          ByRef ret_strResultMsg As String) As Boolean
    Try
      If objPO.EventID = "" Then
        ...
        Return False
      End If
      For Each objPickingDataInfo In objPO.PickingDataList.PickingDataInfo
        '檢查NoticeType是否為空
        If objPickingDataInfo.POId = "" Then
          ret_strResultMsg = "POId is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        '檢查NoticeId是否為空
        If objPickingDataInfo.POType = "" Then
          ret_strResultMsg = "POType is empty"
          SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
          Return False
        End If
        For Each objPickingDetailDataInfo In objPickingDataInfo.PickingDetailDataList.PickingDetailDataInfo
          '檢查NoticeSerialId是否為空
          If objPickingDetailDataInfo.SerialId = "" Then
            ret_strResultMsg = "SerialId is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objPickingDetailDataInfo.SKU = "" Then
            ret_strResultMsg = "SKU is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查SKU是否為空
          If objPickingDetailDataInfo.Unit = "" Then
            ret_strResultMsg = "Unit is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          '檢查QTY是否為空
          If objPickingDetailDataInfo.PickingQty = "" Then
            ret_strResultMsg = "PickingQty is empty"
            SendMessageToLog(ret_strResultMsg, eCALogTool.ILogTool.enuTrcLevel.lvWARN)
            Return False
          End If
          'Dim QTY = CInt(objPickingDetailDataInfo.PickingQty)
          If IsNumeric(objPickingDetailDataInfo.PickingQty) = False Then
  ...
  End Function
  '新增資料或得到要更新的資料
  Private Function Get_UpdateData(ByRef ret_PO_ID As String, ByRef ret_PO_TYPE As String, ByVal objPO_Data As MSG_SendPickingReturnData,
    ...
    Try
      ...
      For Each objPickingDataInfo In objPO_Data.PickingDataList.PickingDataInfo
        Dim PO_ID As String = objPickingDataInfo.POType & "_" & objPickingDataInfo.POId
      ...
      For Each objPickingDataInfo In objPO_Data.PickingDataList.PickingDataInfo
        ...
        Dim PO_ID = objPickingDataInfo.POType & "_" & objPickingDataInfo.POId  '領/退料單號
        ret_PO_ID = objPickingDataInfo.POId
        Dim PO_TYPE1 = ""
        Dim PO_TYPE2 = ""
        Dim PO_TYPE3 = objPickingDataInfo.Kind
        Dim WO_TYPE = ""
        Dim H_PO_ORDER_TYPE = ""
        PickingData_TYPE = objPickingDataInfo.Kind
        ...
        Dim H_PO1 = objPickingDataInfo.POType             '單別
        ret_PO_TYPE = objPickingDataInfo.POType
        Dim H_PO2 = objPickingDataInfo.PickingDateTime    '單據日期
        Dim H_PO3 = objPickingDataInfo.FactoryId          '廠別代號
        Dim H_PO4 = objPickingDataInfo.ProductionLine     '生產線別
        Dim H_PO5 = objPickingDataInfo.Kind               '單據性質
        Dim H_PO6 = objPickingDataInfo.ConfirmCode        '確認碼
        Dim H_PO7 = ""
        Dim H_PO8 = ""
        Dim H_PO9 = ""
        Dim H_PO10 = ""
        ...
        For Each objPO_DTL In objPickingDataInfo.PickingDetailDataList.PickingDetailDataInfo
          ...
          Dim QTY = CDbl(objPO_DTL.PickingQty)
